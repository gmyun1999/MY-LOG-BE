...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--url", type=str, default="http://localhost:3000", help="Grafana URL"
        )
        parser.add_argument(
            "--api-key", type=str, required=True, help="Grafana Admin API Key"
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Health check success: {response.json()}")
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"Could not get version: {response.status_code} - {response.text}"
                    )
                )
        except Exception as e:
        ...
        try:
            ...
            search_response = requests.get(
                f"{grafana_url}/api/serviceaccounts/search", headers=headers
            )

            if search_response.status_code == 200:
                self.stdout.write(
                    self.style.SUCCESS("Service account search API works!")
                )
                accounts = search_response.json()
                self.stdout.write(
                    f"Found {len(accounts.get('serviceAccounts', []))} service accounts"
                )
                ...
                if create_response.status_code in [200, 201]:
                    self.stdout.write(
                        self.style.SUCCESS("Service account creation API works!")
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING("Service account creation API failed!")
                    )
                    ...
                    if create_response.status_code in [200, 201]:
                        self.stdout.write(
                            self.style.SUCCESS(
                                "Service account creation works without role field!"
                            )
                        )
                    else:
                        self.stdout.write(
                            self.style.ERROR(
                                "Service account creation API does not work even without role field!"
                            )
                        )
            else:
                self.stdout.write(
                    self.style.ERROR(
                        f"Service account search API failed: {search_response.status_code} - {search_response.text}"
                    )
                )
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"Service account API test failed: {str(e)}")
            )
        ...
        try:
            folders_response = requests.get(
                f"{grafana_url}/api/folders", headers=headers
            )
            ...
            if folders_response.status_code == 200:
                ...
                folder_response = requests.post(
                    f"{grafana_url}/api/folders", headers=headers, json=folder_data
                )
                ...
                if folder_response.status_code in [200, 201]:
                    ...
                    if folder_uid:
                        ...
                        perm_data = {"items": [{"role": "Viewer", "permission": 0}]}
                        ...
                        if perm_response.status_code in [200, 201]:
                            self.stdout.write(
                                self.style.SUCCESS("Folder permissions API works!")
                            )
                        else:
                            self.stdout.write(
                                self.style.ERROR("Folder permissions API failed!")
                            )
                else:
                    self.stdout.write(
                        self.style.ERROR(
                            f"Folder creation API failed: {folder_response.status_code} - {folder_response.text}"
                        )
                    )
            else:
                self.stdout.write(
                    self.style.ERROR(
                        f"Folder API failed: {folders_response.status_code} - {folders_response.text}"
                    )
                )
        except Exception as e:
        ...
        try:
            ...
            dashboard_response = requests.post(
                f"{grafana_url}/api/dashboards/db", headers=headers, json=dashboard_data
            )
            ...
            if dashboard_response.status_code in [200, 201]:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"Dashboard API failed: {dashboard_response.status_code} - {dashboard_response.text}"
                    )
                )
        except Exception as e:
