...
class GrafanaExecutor(MonitoringDashboardExecutor):
    ...
    def create_service_account(self, user_id: str, role: str = "Viewer") -> str:
        ...
        task_result = TaskResult(
            ...
            status=TaskStatus.PENDING,
            result={"name": account_name, "role": role, "user_id": user_id},
            date_created=now.isoformat(),
            date_started=None,
            date_done=None,
            traceback=None,
    ...
    def set_folder_permissions(self, folder_uid: str, service_account_id: int) -> str:
        ...
        task_result = TaskResult(
            ...
            status=TaskStatus.PENDING,
            result={"folder_uid": folder_uid, "service_account_id": service_account_id},
            date_created=now.isoformat(),
            date_started=None,
            date_done=None,
            traceback=None,
        ...
        set_grafana_folder_permissions.apply_async(
            args=(
                ...
                folder_uid,
                service_account_id,
            ),  # 수정: task_result_id를 첫 번째 인자로 전달
            task_id=saved_result.task_id,
        )
    ...
    def create_dashboard(
        ...
        if folder_uid is None and user_id:
            try:
                from monitoring_provisioner.infra.models.task_result_model import (
                    TaskResultModel,
                )
        ...
        print(
            f"대시보드 생성 요청: dashboard_data={dashboard_data}, folder_uid={folder_uid}"
        )
        return saved_result.id
    ...
    def create_public_dashboard(self, dashboard_uid: str) -> str:
        ...
        create_grafana_public_dashboard.apply_async(
            args=(saved_result.id, dashboard_uid), task_id=saved_result.task_id
        )
