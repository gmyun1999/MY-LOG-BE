...
from monitoring_provisioner.domain.log_agent.agent_provision_context import AgentProvisioningContext
...
class ILogAgentProvider(ABC):
    ...
    @abstractmethod
    def create_log_collector_config(self, context: LogCollectorConfigContext) -> RenderedConfigFile:
        """collector 설정을 render 해서 GeneratedConfig 로 반환"""

    @abstractmethod
    def create_log_router_config(self, context: LogRouterConfigContext) -> RenderedConfigFile:
        """router 설정을 render 해서 GeneratedConfig 로 반환"""

    @abstractmethod
    def create_agent_set_up_script(self, context: AgentProvisioningContext) -> RenderedConfigFile:
