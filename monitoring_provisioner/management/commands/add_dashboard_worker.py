...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--user-id', type=str, required=True, help='User ID to add dashboard for')
        parser.add_argument('--dashboard-title', type=str, default="New Dashboard", help='Title of the dashboard')
        parser.add_argument('--dashboard-template', type=str, help='Path to JSON file with dashboard template')
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"대시보드 추가 시작: 사용자 {user_id}, 제목 '{dashboard_title}'"))
        ...
        if not folder_uid:
            ...
            folder_task_id = executor.create_user_folder(user_id, f"User {user_id}")
            ...
            try:
                with open(dashboard_template, 'r') as f:
                    ...
                    if 'panels' in template_data:
                        ...
                        self.stdout.write(self.style.SUCCESS(f"템플릿에서 {len(panels)}개의 패널을 로드했습니다."))
        ...
        # 3. 대시보드 생성
        self.stdout.write(self.style.SUCCESS(f"대시보드 생성 요청 중 (폴더 UID: {folder_uid})..."))
        ...
        self.stdout.write(self.style.SUCCESS(f"대시보드 생성 완료. UID: {dashboard_uid}"))
        ...
        if public_dashboard_result and 'accessToken' in public_dashboard_result:
            ...
            # 대시보드 URL
            dashboard_url = f"{base_url}/d/{dashboard_uid}/{self.slugify(dashboard_title)}?orgId=1"
            ...
            self.stdout.write(self.style.SUCCESS("이 URL은 로그인 없이 접근할 수 있으며 iframe에 임베드할 수 있습니다."))
            ...
            self.stdout.write(self.style.SUCCESS(f"결과가 {output_file} 파일에 저장되었습니다."))
        else:
