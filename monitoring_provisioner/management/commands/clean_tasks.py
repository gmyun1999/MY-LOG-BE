...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options['all']:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Successfully deleted {count} task results of all statuses')
            )
            return
        ...
        if options['status']:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Successfully deleted {count} task results with status {status}')
            )
            return

        # 기본: FAILURE 및 PENDING 삭제
        failed_count, _ = TaskResultModel.objects.filter(status=TaskStatus.FAILURE).delete()
        pending_count, _ = TaskResultModel.objects.filter(status=TaskStatus.PENDING).delete()
